...
pub(crate) enum AsrBackend {
    ...
    /// 离线 SenseVoice/Whisper：缓冲整段 + 说完后整句转写。
    Offline {
...
impl AsrBackend {
    /// 按模型族构造对应后端（离线族在此被正确接受；`AsrEngine::new` 的离线拒绝守卫只用于其它独立路径）。
    ...
    pub(crate) fn feed_chunk(&mut self, chunk: &[f32], speech_active: bool) {
        match self {
            ...
            Self::Offline { pcm, .. } => pcm.push(chunk, speech_active),
        }
    }
    ...
    /// 强制结束当前句取最终文本：流式走原 force_finalize 逻辑；离线走整段 `transcribe_samples`。
