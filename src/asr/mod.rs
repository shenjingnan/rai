...
use sherpa_onnx::{OnlineRecognizer, OnlineRecognizerConfig, OnlineStream, Wave};
...
impl AsrEngine {
    ...
    pub fn new(cfg: ResolvedAsrConfig) -> Result<Self, String> {
        ...
        c.decoding_method = Some(cfg.decoding_method.clone());
    ...
    /// 识别循环：`while is_ready { decode; get_result → reaction }`。
    ///
    /// 每次 decode 后的结果经 reaction 回调（`is_final` 区分部分/最终文本）；
    /// 端点检测触发（`is_final == true`）时 `reset` 清空流，开始下一句。
    pub fn decode_loop(
        &self,
        stream: &OnlineStream,
...
pub fn run_offline(cfg: &ResolvedAsrConfig, wav: &Path) -> Result<(), String> {
    ...
    engine.decode_loop(&stream, &mut reaction);
    Ok(())
}
